---
title: AI 时代，保持在场 | mhsj.me
description: 关于垂直 AI 产品设计、Agent-First 方法论和 AI Coding 实验的长期观察站，沉淀原创产品思考与实践复盘，而不是短期热点。
socialDescription: 一个关于 AI 产品、Agent-First 和个人产品实验的长期观察站，重点记录原创判断、产品设计思考和实践复盘。
---

你好，我是 mhsj，一名互联网 ToB 产品经理，常年深耕于办公协作工具领域。

近期工作之外，我把大部分精力放在一个问题上：AI 已经彻底改变了软件的构建方式，但多数改变还停留在给旧产品加新功能 —— 而真正因 AI 而存在的新产品形态，应该长什么样？

我选择了做中学、学中做，用 AI Coding 从想法到实践，把手弄脏搞清问题的本质。几个月的密集尝试后，一些零散的想法逐渐收敛抽象成了一个理念，我把它叫做 **Agentic Asset Loop**——Agent 的价值不只在于完成复杂任务，更在于每次运行之后沉淀下来的东西，以及这些沉淀如何让下一次运行更好。

这个站点是思考和实践的同步记录。个人原创在「此山之石」，也收集整理优质内容在「他山之玉」。许多判断仍在摸索中，但我相信，在一个快速变化的领域里，过程及奖励，最重要的就是保持在场、不下牌桌。

## 关于“Agentic Asset Loop”

- [[essays/agentic-asset-loop|Agentic Asset Loop：下一代垂直 AI 产品的设计主张]]
- [[essays/agent-first-products-compounding-ai-outputs|Agent-First 产品怎么做：让 AI 的输出成为复利资产]]
- [[essays/agentic-asset-loop-principles-part-1|Agentic Asset Loop 的 5 条设计原则（上）]]
- [[essays/agentic-asset-loop-principles-part-2|Agentic Asset Loop 的 5 条设计原则（下）]]

## 关于“MCP or CLI”

- [[essays/mcp-or-cli-part-1-deep-dive|MCP or CLI（上）：深度解析]]
- [[essays/mcp-or-cli-part-2-selection-guide|MCP or CLI（中）：选型指南]]

如果你也在思考 AI 时代的产品范式转移，非常欢迎联系我，进一步深入交流～
